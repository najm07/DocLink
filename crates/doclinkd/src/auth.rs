...
pub fn require_auth(
    ...
    let ts: u64 = ts.parse().map_err(|_| StatusCode::UNAUTHORIZED)?;
    let now = unix_now();
    ...
    if grant.expires_unix.map_or(false, |e| e <= now) {
        return Err(StatusCode::FORBIDDEN); // expired
    }

    // The presented key must be the one that was paired.
    let fp = NodeIdentity::fingerprint_from_pubkey_hex(&pk).map_err(|_| StatusCode::UNAUTHORIZED)?;
